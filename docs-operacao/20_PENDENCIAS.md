...
| CONCLUÍDA | FalhasCorrigidas/ — histórico permanente de bugs | — | 33 falhas documentadas em `docs-operacao/FalhasCorrigidas/` (FC001–FC033) com causa raiz, correção, commits e prevenção |
| PENDENTE | FC033 — CancelSubscription não cancela subscription_addons | MÉDIA / Não bloqueante | Decisão de negócio pendente: Opção A (auto-cancelar), B (pausar) ou C (aviso ao usuário). Ver `FalhasCorrigidas/FC033_*.md`. |
