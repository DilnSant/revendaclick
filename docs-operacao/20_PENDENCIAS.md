...
| PENDENTE | **Push dos 6 commits da sessão 64** | **Alta** | `main` local está 6 commits à frente de `origin/main`; produção roda `d512494`. O push dispara deploy da Vercel **e** do VPS (por causa de `onboarding.go`). Bloqueado pela decisão do host canônico abaixo — enviar antes colocaria as landings novas no ar com o defeito de SEO |
| PENDENTE | **Decidir o host canônico do marketing** (`app.` vs apex) | **Alta** | Bloqueia o push. As 6 landings segmentadas declaram canonical em `revendaclick.com.br`, que responde **307** → `www.` → `app.` (verificado ao vivo 06/08/2026). Ver item 0-B em `23_PROXIMO_PASSO.md` |
| PENDENTE | Incluir as 6 landings segmentadas no `sitemap.ts` | Média | `frontend/app/sitemap.ts:9-13` tem lista estática com só `/`, `/privacidade`, `/terms` + vitrines. `/revendas-pequenas`, `/multimarcas`, `/premium`, `/crm-automotivo`, `/erp-automotivo` e `/site-para-revendas` ficam de fora. Depende da decisão do host canônico |
| PENDENTE | Conferência visual das 7 rotas da landing em produção | Média | Nenhuma foi vista em browser — validadas só por `tsc`, `next build` e `eslint`. Fazer após o push |
