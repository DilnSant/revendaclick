...
| PENDENTE | **Decidir o host canônico do marketing** (`app.` vs apex) | **Alta** | ⚠️ **Já está em produção com o defeito** — o push ocorreu antes da decisão. As 6 landings segmentadas declaram canonical em `revendaclick.com.br`, que responde **307** → `www.` → `app.` (verificado ao vivo 06/08/2026). Sem risco operacional; é custo de SEO acumulando enquanto os buscadores rastreiam. Ver item 0-B em `23_PROXIMO_PASSO.md` |
| PENDENTE | Incluir as 6 landings segmentadas no `sitemap.ts` | **Alta** | Subiu para Alta: as rotas estão no ar mas invisíveis no sitemap. `frontend/app/sitemap.ts:9-13` tem lista estática com só `/`, `/privacidade`, `/terms` + vitrines. Depende da decisão do host canônico |
| PENDENTE | Conferência visual das 7 rotas da landing em produção | **Alta** | Estão no ar e respondem HTTP 200, mas **nenhuma foi vista em browser**. Validadas só por `tsc`, `next build`, `eslint` e código de status. Um erro de layout ou copy estaria visível ao público agora |
