...
| PARCIAL | Conferência visual das 7 rotas da landing em produção | **Alta** | Sessão 66: a home (`/`) foi conferida via screenshot Playwright contra produção como parte da validação do D40 (nova seção de preços) — achou e corrigiu o vazamento visual do modal Enterprise (FC/commit `ca7a0ce`). **As 6 landings segmentadas** (`/revendas-pequenas`, `/multimarcas`, `/premium`, `/crm-automotivo`, `/erp-automotivo`, `/site-para-revendas`) **ainda não foram vistas em browser** |
