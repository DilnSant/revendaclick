...
func New(cfg *config.Config, pool *pgxpool.Pool, logger *zap.Logger) http.Handler {
	...
	adminGroup.Use(jwtAuth, superAdmin)
	{
		...
		adminGroup.POST("/tenants/:id/quarantine",             adminH.QuarantineTenant)
		adminGroup.POST("/tenants/:id/unquarantine",           adminH.UnquarantineTenant)
		adminGroup.GET("/tenants/:id/delete-summary",          adminH.GetDeleteSummary)
		adminGroup.DELETE("/tenants/:id",                      adminH.DeleteTenant)
